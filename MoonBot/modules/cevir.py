...
async def cevir(event):
    ...
    if islem == "foto" or islem == "photo":
        ...
        await event.client.send_file(event.chat_id, "sticker.png", reply_to=rep_msg, caption="@OwenMoonBot `ile Fotoğrafa çevirildi`")
        ...
        if efekt in EFEKTLER:
            ...
            await event.client.send_file(event.chat_id, "output.mp3", reply_to=rep_msg, caption="@OwenMoonBot `ile efekt uygulandı.`")
        ...
        try:
            await event.client.send_file(event.chat_id, "out.gif",reply_to=rep_msg, caption=LANG['WITH_OWEN_GIF'])
        except:
        ...
        try:
            await event.client.send_file(event.chat_id, "out.mp3",reply_to=rep_msg, caption='@OwenMoonBot ile sese çevrildi.')
        except:
            os.remove(video)
