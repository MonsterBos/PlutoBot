...
from MoonBot import DEFAULT_NAME
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        fpath_ = glob.glob("MoonBot/fonts/*")
    ...
    if os.path.exists(fayladi):
        await event.client.send_file(
            event.chat_id,
            file=fayladi,
            caption=f"[OwenMoonBot](https://t.me/owenMoonBot) **{DEFAULT_NAME}** için logo hazırladı.",
            force_document=True,
        )
        os.remove(fayladi)
        await event.delete()
    ...
    if os.path.exists(font_):
        if not font_.startswith("MoonBot/fonts/"):
            os.remove(font_)

Help = CmdHelp('logo')
Help.add_command('logo', '<yazı>', '@OwenMoonBot sizin için logo hazırlar.')
Help.add_info('@OwenMoonBot - @ByMisakiMey ')
Help.add() 
