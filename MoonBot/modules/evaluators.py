...
from MoonBot import CMD_HELP, BOTLOG, BOTLOG_CHATID
...
async def evaluate(query):
    ...
    if (expression in ("MoonBot.session", "config.env")) or (expression == 'env'):
...
async def run(run_q):
    ...
    if (code in ("MoonBot.session", "config.env")) or code is 'env':
...
async def terminal_runner(term):
    ...
    if command in ("MoonBot.session", "config.env", "env"):
