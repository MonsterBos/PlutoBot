...
from MoonBot.events import register as r
from MoonBot.cmdhelp import CmdHelp as c
			...
			if sleep_t == "":
				...
				if sec == "":
					...
					if sec.lower() == "edit":
						...
						import MoonBot.helpers.p as edit
						edit.e_(dosya_name, name, slep, liste)
						#file = await q.client.upload_file(f'./fg{dosya_name}.py')
						await q.client.send_file(q.chat_id, f"./owenMoonBot{dosya_name}.py", force_document=True, caption="Bu Plugin @OwenMoonBot Tarafından Yapılmıştır..")
						await q.delete()
						os.remove(f"./owenMoonBot{dosya_name}.py")
						return
...
c_ = c("pluginmaker")
c_.add_command("pmake", "<pluginin_komutu> <plugin_hızı> <edit/alt/foto/muzik> ", "@OwenMoonBot Sizin İçin Bir plugin Oluşturur...")
