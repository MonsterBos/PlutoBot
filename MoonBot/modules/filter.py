...
""" Filtre komutlarını içeren MoonBot modülüdür. """

from asyncio import sleep
import re
from MoonBot import BOTLOG, BOTLOG_CHATID, CMD_HELP, BLACKLIST_CHAT
...
async def filter_incoming_handler(handler):
    ...
    try:
        if not (await handler.get_sender()).bot:
            try:
                from MoonBot.modules.sql_helper.filter_sql import get_filters
...
async def genelfilter(event):
    try:
        from MoonBot.modules.sql_helper.filter_sql import add_filter
    except AttributeError:
...
async def remove_a_genel(r_handler):
    ...
    try:
        from MoonBot.modules.sql_helper.filter_sql import remove_filter
    except AttributeError:
