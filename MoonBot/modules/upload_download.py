...
from MoonBot import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY, BOT_USERNAME
...
async def download(target_file):
    """ .download komutu MoonBot sunucusuna dosya indirmenizi sağlar. """
...
async def upload(u_event):
    """ .upload komutu MoonBot sunucusundan dosya uploadlamaya yarar. """
    await u_event.edit(LANG['TRYING'])
    input_str = u_event.pattern_match.group(1)
    if input_str in ("MoonBot.session", "config.env"):
