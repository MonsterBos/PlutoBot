...
from MoonBot import CMD_HELP, bot, tgbot, PLUGIN_CHANNEL_ID, PATTERNS, BOTLOG, BOTLOG_CHATID, ASISTAN, MYID, DEFAULT_NAME
from telethon.tl.types import InputMessagesFilterDocument
from MoonBot.events import register
from MoonBot.main import extractCommands
import MoonBot.cmdhelp
...
async def pport(event):
    ...
    if re.search(borg1, dosy):
        ...
        degistir = degistir.replace("from MoonBot.utils import admin_cmd", "from MoonBot.events import register")
        degistir = re.sub(r"(from uniborg).*", "from MoonBot.events import register", degistir)
...
async def pins(event):
    ...
    await event.edit(LANG["DOWNLOADING"])
    edizin = f"./MoonBot/modules/{reply_message.file.name}"
 ...
 #       return
    ...
    dosya = await event.client.download_media(reply_message, "./MoonBot/modules/")
    ...
    try:
        ...
        return os.remove("./MoonBot/modules/" + dosya)
    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                MoonBot.cmdhelp.CmdHelp(dosya).add_warning('Komutlar bulunamadı!').add()
...
async def ptest(event):
    ...
    await event.edit(LANG["DOWNLOADING"])
    if not os.path.exists('./MoonBot/temp_plugins/'):
        os.makedirs('./MoonBot/temp_plugins')
    dosya = await event.client.download_media(reply_message, "./MoonBot/temp_plugins/")
    ...
    try:
        ...
        await event.edit(f"{LANG['PLUGIN_BUGGED']} {e}`")
        return os.remove("./MoonBot/temp_plugins/" + dosya)
    plugin = await event.get_reply_message()
    dosy = open(dosya, "r").read()
    for S in SECURİTY:
     if re.search(S, dosy):
...
async def psend(event):
    ...
    if os.path.isfile(f"./MoonBot/modules/{modul}.py"):
        await event.client.send_file(event.chat_id, f"./MoonBot/modules/{modul}.py", caption=LANG['OWEN_PLUGIN_CAPTION'])
        await event.delete()
    else:
...
async def premove(event):
    ...
    async for message in event.client.iter_messages(PLUGIN_CHANNEL_ID, filter=InputMessagesFilterDocument, search=modul):
        ...
        try:
            os.remove(f"./MoonBot/modules/{message.file.name}")
...
async def pinsasistan(ups):
    ...
    dosya = await ups.client.download_media(reply_message, "./MoonBot/modules/")
