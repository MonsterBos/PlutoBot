...
from MoonBot import (BIO_PREFIX, BOTLOG, BOTLOG_CHATID, CMD_HELP,
    ...
    while SPOTIFYCHECK:
        try:
            ...
            if song != oldsong and artist != oldartist:
                ...
                spobio = "@OwenMoonBot 🎧: " + artist + " - " + song
                try:
...
def get_spotify_info(TIME=5):
    try:
        ...
        if len(item['album']['images']) > 0:
            ...
            if path.exists("@OwenMoonBot-Spotify.jpg"):
                remove("@OwenMoonBot-Spotify.jpg")          
            try:
                r = get(str(item['album']['images'][0]['url']))
                with open("@OwenMoonBot-Spotify.jpg", 'wb') as f:
                    f.write(r.content)    

                with open('OwenMoonBot-Spotify.jpg', 'rb') as f:
