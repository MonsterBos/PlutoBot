...
from MoonBot import BOT_USERNAME, PATTERNS, CMD_HELP, PLUGIN_CHANNEL_ID
...
async def magaza(event):
    ...
    if plugin == '':
        plugin = 'Son Yüklenen'
        plugins = await event.client.get_messages('@owenplugin', limit=15, filter=InputMessagesFilterDocument)
    elif len(split) >= 1 and (split[0] == 'random' or split[0] == 'rastgele'):
        plugin = 'Rastgele'
        plugins = await event.client.get_messages('@owenplugin', limit=None, filter=InputMessagesFilterDocument)
        plugins = sample(plugins, int(split[1]) if len(split) == 2 else 5)
    else:
        plugins = await event.client.get_messages('@owenplugin', limit=None, search=plugin, filter=InputMessagesFilterDocument)
        random = await event.client.get_messages('@owenplugin', limit=None, filter=InputMessagesFilterDocument)
...
async def sinstall(event):
    ...
    dosya = await plugin.download_media('./MoonBot/modules/')
    ...
    try:
        ...
        os.remove("./MoonBot/modules/" + dosya)
...
MoonBot.cmdhelp.CmdHelp('store').add_command(
