...
for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "owenjson")):
        if path.isfile(f"./MoonBot/language/{dil.file.name}"):
            try:
                LANGUAGE_JSON = loads(open(f"./MoonBot/language/{dil.file.name}", "r").read())
            except JSONDecodeError:
                dil.delete()
                remove(f"./MoonBot/language/{dil.file.name}")

                if path.isfile("./MoonBot/language/DEFAULT.owenjson"):
                    LOGS.warn("Varsayılan dil dosyası kullanılıyor...")
                    LANGUAGE_JSON = loads(open(f"./MoonBot/language/DEFAULT.owenjson", "r").read())
                else:
                    raise Exception("Your language file is invalid")
        else:
            try:
                DOSYA = dil.download_media(file="./MoonBot/language/")
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./MoonBot/language/{LANGUAGE}.owenjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./MoonBot/language/{LANGUAGE}.owenjson", "r").read())
        except JSONDecodeError:
