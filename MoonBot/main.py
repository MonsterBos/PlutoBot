...
import MoonBot.modules.sql_helper.mesaj_sql as MSJ_SQL
import MoonBot.modules.sql_helper.galeri_sql as GALERI_SQL
...
def extractCommands(file):
    ...
    if re.search(r'CmdHelp\(.*\)', FileRead):
        ...
        CmdHelp = MoonBot.cmdhelp.CmdHelp(dosyaAdi, False)
    ...
    if not PLUGIN_CHANNEL_ID == None:
        ...
        for plugin in bot.iter_messages(KanalId, filter=InputMessagesFilterDocument):
            if plugin.file.name and (len(plugin.file.name.split('.')) > 1) \
                ...
                if not os.path.exists("./MoonBot/modules/" + plugin.file.name):
                    dosya = bot.download_media(plugin, "./MoonBot/modules/")
                else:
                    LOGS.info("Bu Plugin Onsuzda Yüklüdür " + plugin.file.name)
                    extractCommands('./MoonBot/modules/' + plugin.file.name)
                    dosya = plugin.file.name
                    continue 
                
                try:
                    spec = importlib.util.spec_from_file_location("MoonBot.modules." + Split[0], dosya)
                    ...
                    if os.path.exists("./MoonBot/modules/" + plugin.file.name):
                        os.remove("./MoonBot/modules/" + plugin.file.name)
                    continue
                extractCommands('./MoonBot/modules/' + plugin.file.name)
    else:
...
for module_name in ALL_MODULES:
    imported_module = import_module("MoonBot.modules." + module_name)
if BOT_TOKEN:
 for module_name in ALL_MODULE:
    imported_module = import_module("MoonBot.asisstant.modules." + module_name)    
